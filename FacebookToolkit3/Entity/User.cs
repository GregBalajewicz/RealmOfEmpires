...
namespace Facebook.Entity {
    ...
    public enum LookingFor
    {
        ...
        RandomPlay,
#if !NETCF
        [Description("Whatever I Can Get")]
#endif
        WhateverIcanget,
#if !NETCF
    ...
    public class User
    {
        ...
        /// Default constructor
        /// </summary>
        public void LoadFromDic(Dictionary<String, Facebook.JSONObject> dic)
        {
            UserId = dic["id"].String;
            Name = dic["name"].String;
            FirstName = IfContains(dic, "first_name");
            LastName = IfContains(dic, "last_name");
            Email = IfContains(dic, "email");
            ProfileLink = IfContains(dic, "link");
           

            if (dic.ContainsKey("gender"))
            {
                if (Enum.IsDefined(typeof(Gender), dic["gender"].String))
                {
                    Sex = (Gender)Enum.Parse(typeof(Gender), dic["gender"].String, true);
                }
                else
                {
                    switch (dic["gender"].String.ToUpper())
                    {
                        case "MALE":
                            Sex = Gender.Male;
                            break;
        ...
        private string IfContains(Dictionary<String, Facebook.JSONObject> dic, string name)
        {
            return IfContains(dic, name, string.Empty);
        }
        private string IfContains(Dictionary<String, Facebook.JSONObject> dic, string name, string ifNotFoundValue)
        {
            return dic.ContainsKey(name) ? dic[name].String : ifNotFoundValue; 
        }
