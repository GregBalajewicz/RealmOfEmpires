...
namespace Facebook.Parser {
    internal sealed class FacebookEventParser
    {
        /// <summary>
        /// The default constructor
        /// </summary>
        private FacebookEventParser() { }
        ...
        internal static FacebookEvent ParseEvent(XmlNode node)
        {
            ...
            if (node != null)
            {
                ...
                {
                    ev.UpdateDate = DateHelper.ConvertDoubleToDate(double.Parse(XmlHelper.GetNodeText(node, "update_time"), CultureInfo.InvariantCulture));
                }
                if (!String.IsNullOrEmpty(XmlHelper.GetNodeText(node, "start_time")))
                {
                    ev.StartDate = DateHelper.ConvertDoubleToEventDate(double.Parse(XmlHelper.GetNodeText(node, "start_time"), CultureInfo.InvariantCulture));
                }
                if (!String.IsNullOrEmpty(XmlHelper.GetNodeText(node, "end_time")))
                {
                    ev.EndDate = DateHelper.ConvertDoubleToEventDate(double.Parse(XmlHelper.GetNodeText(node, "end_time"), CultureInfo.InvariantCulture));
                }
