IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'dAccountStewardship')
BEGIN
	DROP  Procedure  dAccountStewardship
END

GO

--
-- this is when a steward cancel the stewardship of someone elses account
--
CREATE Procedure dbo.dAccountStewardship
		...
		IF @AccountOwnerPlayerID is not null BEGIN
			...
			IF @@rowcount >= 1 begin 
				insert into AccountStewardLog(ActingPlayerId, EventTypeID, Time, Notes)
					values (@AccountOwnerPlayerID , 4 , getdate(), @StewardPlayerID)
			END 
		END
	COMMIT TRAN
end try
begin catch
	...
	SET @ERROR_MSG = 'dAccountStewardship FAILED! ' +  + CHAR(10)
