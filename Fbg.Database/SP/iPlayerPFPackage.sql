  IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'iPlayerPFPackage')
	BEGIN
		DROP  Procedure  iPlayerPFPackage
	END

GO

CREATE Procedure dbo.iPlayerPFPackage
	@PlayerID as int,
	@PackageID as int
AS
begin try  
	...
	IF exists (select * from PlayersPFPackages where PlayerID = @PlayerID and  PFPackageID = @PackageID ) BEGIN
		EXEC uPlayerPFPackage @PlayerID, @PackageID
	END ELSE BEGIN
		
		select @Cost=Cost ,@Duration=Duration from PFPackages where PFPackageID=@PackageID
		...
		if @Duration < 1 BEGIN
			SET @ExpiresOn = DATEADD(hour, round(@Duration  * 24,0),GETDATE())
		END ELSE BEGIN 
			SET @ExpiresOn = DATEADD(day,@Duration,GETDATE())
		END

		insert into PlayersPFPackages 
			(PlayerID,PFPackageID,ExpiresOn) 
		values
			(@PlayerID,@PackageID, @ExpiresOn)
		
		-- log the process of adding new package
		insert into PlayerPFLog
			(PlayerID,Time ,EventType,Credits ,Cost,notes)
			values
			(@PlayerID,getdate(),2,@Cost,-1, @PackageID)
		select @Cost
	END
  end try
begin catch
	...
	SET @ERROR_MSG = 'iPlayerPFPackage FAILED! ' +  + CHAR(10)
