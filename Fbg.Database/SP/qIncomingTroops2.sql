 IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'qIncomingTroops2')
	BEGIN
		DROP  Procedure  qIncomingTroops2
	END
...
*/
CREATE Procedure qIncomingTroops2
	@ToPlayerID as int
...
select UM.EventID			as EventID
	...
	, (CASE CommandType WHEN 2 THEN TripDuration
						WHEN 3 THEN TripDuration
		ELSE 0			
		END)
		as TripDuration
	into #mov
...
select distinct OriginVillageID, OriginVillageName, OriginVillageX, OriginVillageY from #mov   
	where OriginVillageName <> '' -- remove "unknown origin" incoming
