IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'tbh_Forums_GetSharedForumsByClanID')
	BEGIN
		DROP  Procedure  tbh_Forums_GetSharedForumsByClanID
	END
...
GO
 CREATE PROCEDURE [dbo].[tbh_Forums_GetSharedForumsByClanID]
(
   @ClanID  int,
   @PlayerID int ,
