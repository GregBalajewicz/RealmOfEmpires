  ...
  IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'uSpeedUpUpgrade_viaItem')
	BEGIN
		DROP  Procedure  uSpeedUpUpgrade_viaItem
	END

GO

--
-- If successful, this returns 1 in @success
--  if not succesful, @Cost is zero or null
--
CREATE Procedure uSpeedUpUpgrade_viaItem
		@EventID bigint
		, @MinToCut int
		, @PlayerID int 
        , @success int OUTPUT
...
AS
	...
	END ELSE BEGIN
		...
		SET NOCOUNT ON
	END 
	IF @DEBUG = 1 SELECT 'BEGIN uSpeedUpUpgrade_viaItem ' + cast(@EventID as varchar(10))
	...
	begin try 	
		...
		IF @DEBUG = 1 SELECT 'END uSpeedUpUpgrade_viaItem ' + cast(@EventID as varchar(10))
...
begin catch
	...
	SET @ERROR_MSG = 'uSpeedUpUpgrade_viaItem FAILED! ' +  + CHAR(10)
