<%@ Page Language="C#" MasterPageFile="~/main.master" AutoEventWireup="true" CodeFile="ManageSharedForums.aspx.cs"
    Inherits="ManageSharedForums" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <%if (isMobile) { %><a href="manageforums.aspx">Back to Clan Forum Admin</a> <Br></Br><BR> <%} %>
    <asp:GridView ID="gvwMySharedForums" runat="server" AutoGenerateColumns="False" DataKeyNames="ID"
        CellPadding="1" CellSpacing="1" GridLines="none" ShowHeader="True" CssClass="TypicalTable stripeTable"
        OnRowCommand="gvwMySharedForums_RowCommand" 
        OnRowDataBound="gvwMySharedForums_RowDataBound" 
        onselectedindexchanged="gvwMySharedForums_SelectedIndexChanged">
        <Columns>
            ...
            <asp:TemplateField HeaderText="Name and Description">
                ...
                <ItemTemplate>
                    <div class="sectionsubtitle">
                        <asp:Literal runat="server" ID="lblClanName" Text='<%# Eval("Title") %>' />
                    </div>
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="Shared With Clans">
                <HeaderTemplate>
                    <%= RS("sharedWithClans")%>
                </HeaderTemplate>
                <ItemTemplate>
                    <asp:HiddenField runat="server" ID="hdn_ForumID" Value='<%# Eval("ID") %>'></asp:HiddenField>
                    <asp:DataList runat="server" ID="dl_Clans" RepeatDirection="Horizontal" OnItemCommand="dl_Clans_ItemCommand">
                        <ItemTemplate>
                            <asp:Literal ID="Literal1" runat="server" Text='<%# Eval("Name") %>'></asp:Literal>
                            
                            <asp:ImageButton ID="ImageButton1" CommandArgument='<%# Eval("ClanID")+","+  Eval("ForumID")%>' CommandName="Remove"
                                runat="server" ImageUrl='https://static.realmofempires.com/images/cancel.png' />
                        </ItemTemplate>
                    </asp:DataList>
                    <asp:Panel ID="pnl_Add" runat="server" Visible="false">
                        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="border: solid 1px black">
                            <tr>
                           ...
                           <%= RS("WLDesc")%>         
                            ...
                            <tr>
                                <td style="padding: 1px;">
                                    <asp:TextBox ID="txt_ClanName" runat="server" CssClass="TextBox jsclans" />
                                    
                                    <asp:Button CssClass="inputbutton" Text='Add' ID="lnk_Add" runat="server" CommandArgument='<%# Eval("ID") %>' CommandName="Add" 
                                        ValidationGroup="vg_Add" />
                                    <asp:Label ID="lbl_Error" CssClass="Error" runat="server" Visible="False" />
                                    &nbsp;
                                </td>
                                <td>
                            </tr>
        ...
        <EmptyDataTemplate>
            <b>
                <%= RS("noClans")%></b></EmptyDataTemplate>
    </asp:GridView>
</asp:Content>
