...
function pieChart() {
    ...
    var pullOutShadowColour = "rgba( 0, 0, 0, .4 )";  // Colour to use for the pull-out slice shadow
    ...
    var pullOutShadowBlur = 2;                        // How much to blur the pull-out slice shadow
    ...
    */
    ...
    function handleChartClick(clickEvent) {
        ...
        var mouseX = clickEvent.clientX - this.offsetWidth;
        var mouseY = clickEvent.clientY - this.offsetHeight;

        // Bug: Position of cursor relative to element should be calculated in
        // the following way; however, jQuery seems to be unable to get or set
        // offset of canvas correctly (can be seen by setting offset of canvas
        // to (0, 0) repeatedly; the position will keep changing).
//        var mouseX = clickEvent.pageX - $(this).offset.left;
//        var mouseY = clickEvent.pageY - $(this).offset.top;

        // Workaround: For above-mentioned bug
        var box = this.getBoundingClientRect();
        var mouseX = clickEvent.offsetX || clickEvent.pageX - box.left - window.scrollX;
        var mouseY = clickEvent.offsetY || clickEvent.pageY - box.top - window.scrollY;
        ...
        if (distanceFromCentre <= chartRadius) {
            ...
            for (var i = 0; i < chartData.length; i++) {
                ...
                if (clickAngle >= chartData[i]['startAngle'] && clickAngle <= chartData[i]['endAngle']) {
    ...
    */
    ...
    function drawChart() {
        ...
        for (var i = 0; i < chartData.length; i++) {
            if (i != currentPullOutSlice) drawSlice(context, i);
        }
...
//            context.fillStyle = 'rgb(255,255,255)';//  + chartColours[slice].join(',') + ')';
...
//            context.shadowBlur = pullOutShadowBlur;
        ...
        if (slice == currentPullOutSlice) {
          ...
          if (slice == currentPullOutSlice) {
            ...
            context.fillStyle = 'rgb(0,0,0)';//  + chartColours[slice].join(',') + ')';
            context.textAlign = "center";
            context.font = pullOutLabelFont;
            context.fillText(chartData[slice]['label'], centreX + Math.cos(midAngle) * (chartRadius + maxPullOutDistance + pullOutLabelPadding), centreY + Math.sin(midAngle) * (chartRadius + maxPullOutDistance + pullOutLabelPadding));
            context.font = pullOutValueFont;
